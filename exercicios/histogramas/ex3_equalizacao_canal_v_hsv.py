...
# ── Conversão BGR → HSV ───────────────────────────────────────────────────────
# HSV separa a COR (Hue) do BRILHO (Value) e da SATURAÇÃO
# Vantagem: equalizar SOMENTE V ajusta o contraste sem alterar as cores
# (equalizando BGR separado, cada canal muda na sua proporção → cores distorcidas)
imagem_hsv = cv2.cvtColor(imagem_bgr, cv2.COLOR_BGR2HSV)

# Separar os canais H (matiz 0-179), S (saturação 0-255) e V (brilho 0-255)
h, s, v = cv2.split(imagem_hsv)

# ── Equalizar apenas o canal V ────────────────────────────────────────────────
# cv2.equalizeHist(v): redistribui as intensidades do canal V para cobrir [0-255]
# H e S NÃO são modificados → cores originais preservadas → resultado natural
v_equalizado = cv2.equalizeHist(v)

# cv2.merge(): recombina os 3 canais em uma imagem HSV novamente
hsv_equalizado = cv2.merge([h, s, v_equalizado])

# ── Converter de volta para BGR ───────────────────────────────────────────────
# COLOR_HSV2BGR: inverso da conversão inicial; resultado pronto para salvar/exibir
