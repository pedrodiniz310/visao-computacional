...
# cv2.createCLAHE: cria objeto CLAHE reutilizável (mais eficiente que recriar por frame)
# clipLimit=2.0: limitação padrão — equilíbrio entre contraste e ruído
# tileGridSize=(8,8): 64 tiles por frame (troca entre qualidade e velocidade)
clahe = cv2.createCLAHE(clipLimit=2.0, tileGridSize=(8, 8))

# ── Abertura do vídeo ─────────────────────────────────────────────────────────
# cv2.VideoCapture(caminho): abre arquivo de vídeo (aceita também índice de câmera: 0)
cap = cv2.VideoCapture(PATH_VIDEO)
if not cap.isOpened():
    raise IOError(f'Não foi possível abrir o vídeo: {PATH_VIDEO}')

# CAP_PROP_FPS: frames por segundo originais do arquivo de vídeo
fps_video = cap.get(cv2.CAP_PROP_FPS)
# Atraso para waitKey: garante que a janela atualize na cadência original do vídeo
