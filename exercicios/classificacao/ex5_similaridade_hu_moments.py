...
IMAGENS = {
    'disco1'   : os.path.normpath(os.path.join(os.path.dirname(__file__), '../../images/disco1.png')),
    'disco2'   : os.path.normpath(os.path.join(os.path.dirname(__file__), '../../images/disco2.png')),
    'disco3'   : os.path.normpath(os.path.join(os.path.dirname(__file__), '../../images/disco3.png')),
    ...
    'puzzle'   : os.path.normpath(os.path.join(os.path.dirname(__file__), '../../images/puzzle.png')),
}
...
def extrair_hu_moments(imagem_cinza: np.ndarray) -> np.ndarray:
    ...
    Os Momentos de Hu são 7 valores invariantes a:
      - Translação (centroide não afeta)
      - Escala (normalizados pela área)
      - Rotação (M7 também é invariante à reflexão, exceto H7)
    Permitem comparar formas geométricas independente de posição/tamanho/orientação.
    """
    suave = cv2.GaussianBlur(imagem_cinza, (5, 5), 0)
    # THRESH_BINARY_INV + OTSU: objeto escuro → branco, fundo → preto
    _, binaria = cv2.threshold(suave, 0, 255,
                                cv2.THRESH_BINARY_INV + cv2.THRESH_OTSU)

    # Garantir que o objeto seja branco e o fundo preto
    # (se a média > 127, a maioria dos pixels é branca → fundo branco → inverter)
    if np.mean(binaria) > 127:
        binaria = cv2.bitwise_not(binaria)

    # Morfologia para limpar resíduos de binarização
    ...
    # Encontrar o maior contorno (objeto principal — ignora ruído residual)
    ...
    # cv2.moments(): calcula todos os momentos geométricos do contorno
    M = cv2.moments(cnt_maior)

    # cv2.HuMoments(M): calcula os 7 Momentos de Hu a partir dos momentos M
    # np.sign(hu) * log10(|hu|): transformação log para equalizar a escala
    # (os 7 valores têm magnitudes muito diferentes, de 10^-2 a 10^-25)
