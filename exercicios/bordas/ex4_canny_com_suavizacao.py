...
IMAGENS = {
    'monumento': os.path.normpath(os.path.join(os.path.dirname(__file__), '../../images/monumento.png')),
    'Noise_SP' : os.path.normpath(os.path.join(os.path.dirname(__file__), '../../images/Noise_SP.png')),
}
...
def processar(caminho: str, nome: str) -> None:
    ...
    # ── Versões suavizadas ────────────────────────────────────────────────────
    # GaussianBlur reduz ruído gaussiano/fundo; preserva bordas mais que o blur simples
    # sigmaX=0: calcula sigma automaticamente a partir de ksize (5) → sigma≈1.1
    gaussiano = cv2.GaussianBlur(cinza, (5, 5), 0)
    # medianBlur: melhor para ruído sal-e-pimenta (não-linear, ignora outliers)
    mediana   = cv2.medianBlur(cinza, 5)

    # ── Canny em cada versão ─────────────────────────────────────────────────────────
    # Mesmos limiares T1 e T2 para todos os casos → comparação justa
    # O Canny internamente já aplica um Gaussiano 5×5; aplicar outro antes
    # aumenta ainda mais a suavização → menos pixels de borda falsos
