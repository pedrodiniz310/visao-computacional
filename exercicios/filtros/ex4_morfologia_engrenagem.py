...
# Elipse é boa opção para formas circulares como engrenagens
kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (5, 5))

# ── Operações morfológicas ────────────────────────────────────────────────────

# 1. ABERTURA (Erosão → Dilatação):
#    Remove pontos brancos pequenos (ruído) no fundo sem alterar muito o objeto
abertura = cv2.morphologyEx(binaria, cv2.MORPH_OPEN, kernel)

# 2. FECHAMENTO (Dilatação → Erosão):
#    Preenche buracos pequenos dentro do objeto (imperfeições internas)
