...
# ── Aplicar Mediana com diferentes kernels ────────────────────────────────────# cv2.medianBlur(src, ksize):
#   Substitui cada pixel pelo VALOR DO MEIO (mediana) após ordenar os K×K vizinhos.
#   A mediana é ROBUSTA a outliers: pixels de ruído (0 ou 255) são minoría no grupo
#   → ignorados na ordenação → eliminados sem espalhamento no fundo.
#   ksize DEVE ser ímpar: 3, 5, 7, 9 ... (para ter pixel central definido)kernels    = [3, 5, 9]
